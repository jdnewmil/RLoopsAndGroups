...
```{r}
DF2$ans <- with( DF2, my_solution( A, B, C ) )
head( DF2 )
```

Each row now has an answer associated with it. How to present?
...
```{r}
DF2$Cgroup <- factor( DF2$C, levels = Cvals )
head( DF2 )
```
...
Ggplot will separate the data into groups of records where the `Cgroup` column values are all the same. Note that because `C` is numeric, `ggplot` would assume there could be values like 3.7 even though we purposely skipped over that value. 
...
- Vectorized code uses all vector-based primitive operations
    + `apply`-type code _looks_ vectorized so easy to read, but not fast
